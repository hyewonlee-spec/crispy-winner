...
function sessionsInLastDays(sessions,days){
  ...
  return (sessions||[]).filter(s=>{const d=getSessionDateValue(s); if(!d) return false; const diff=(today-d)/(1000*60*60*24); return diff>=0 && diff<days;});
}
...
function ProgressLineChart({title,data,keyName}){
  const now=new Date(); const today=new Date(now.getFullYear(),now.getMonth(),now.getDate());
  const days=Array.from({length:21},(_,i)=>{const d=new Date(today); d.setDate(today.getDate()-(20-i)); return d;});
  const dayKey=d=>d.toISOString().slice(0,10); const byDay=new Map();
  (data||[]).forEach(s=>{if(!s.date||typeof s[keyName]!=="number")return; if(!byDay.has(s.date))byDay.set(s.date,[]); byDay.get(s.date).push(s[keyName]);});
  const values=days.map(d=>{const vals=byDay.get(dayKey(d))||[]; return vals.length?Math.round((vals.reduce((a,b)=>a+b,0)/vals.length)*10)/10:null;});
  const w=320,h=150,padL=34,padR=12,padT=14,padB=24; const xFor=i=>padL+(i*(w-padL-padR))/(days.length-1); const yFor=v=>padT+((10-v)/9)*(h-padT-padB);
  const points=values.map((v,i)=>v===null?null:${xFor(i)},${yFor(Math.max(1,Math.min(10,v)))});
  const segments=[]; let current=[]; points.forEach(point=>{if(point)current.push(point); else if(current.length){segments.push(current); current=[]}}); if(current.length)segments.push(current);
  return <div className="chart progressReportChart"><h3>{title}</h3><svg viewBox={`0 0 ${w} ${h}} className="lineChart progressLineChart" preserveAspectRatio="none">
    ...
    <text x="8" y={yFor(10)+4} className="axisText">10</text><text x="14" y={yFor(1)+4} className="axisText">1</text>
    ...
    {points.map((point,i)=>{if(!point)return null; const [x,y]=point.split(",").map(Number); return <circle key={i} cx={x} cy={y} r="4"/>})}
  </svg></div>
}
function WeeklyMonthlyOverview({sessions}){
  const week=sessionsInLastDays(sessions,7); const month=sessionsInLastDays(sessions,28);
  return <div className="overviewGrid"><div className="panel miniOverview"><h3>Weekly overview</h3><p>Avg energy level: <b>{avgMetric(week,"energy")}</b></p><p>Avg stress level: <b>{avgMetric(week,"stress")}</b></p><p>Avg sleep quality: <b>{avgMetric(week,"sleep")}</b></p></div><div className="panel miniOverview"><h3>Monthly overview</h3><p>Avg energy level: <b>{avgMetric(month,"energy")}</b></p><p>Avg stress level: <b>{avgMetric(month,"stress")}</b></p><p>Avg sleep quality: <b>{avgMetric(month,"sleep")}</b></p></div></div>
}
...
export default function App(){
  ...
  <Button variant="primary" full busy={busy.save} onClick={recordDayAndPromptWorkout}><Save size={16}/> {dayRecordedToday ? "Start workout" : "Record and start workout"}</Button>
    ...
    {tab==="progress"&&<section className="stack progressReport"><div className="panel"><div className="sectionTitle"><BarChart3 size={20}/><h2>Progress Report</h2></div><Button variant="secondary" full busy={busy.history} onClick={()=>loadHistory(true)}>Refresh progress</Button>{history.error&&<p className="errorText">{history.error}</p>}</div><WeeklyMonthlyOverview sessions={history.sessions}/><ProgressLineChart title="Energy level" data={history.sessions} keyName="energy"/><ProgressLineChart title="Stress level" data={history.sessions} keyName="stress"/><ProgressLineChart title="Sleep quality" data={history.sessions} keyName="sleep"/><div className="panel"><h2>Recent sessions</h2><div className="sessionList">{history.sessions.slice(0,12).map(s=><div className="sessionItem" key={s.id}><div className="row"><b>{s.date||"No date"}</b><span className="pill">{s.type||"Log"}</span></div><p className="muted">Sleep {s.sleep??"—"} · Energy {s.energy??"—"} · Stress {s.stress??"—"}</p>{s.notes&&<p className="note">{s.notes}</p>}</div>)}</div></div></section>}
  ...
  <div className="panel cyclePanel">
    ...
    {trackCycle==="yes"&&<div className="panel"><div className="sectionTitle"><Sparkles size={20}/><h2>Cycle-aware training</h2></div><Button variant="secondary" full onClick={()=>setTab("cycle")}>Open Cycle tab</Button></div>}<div className="checkGrid"><div className="panel checkCard"><div className="sectionTitle"><Soup size={20}/><h3>Food choice</h3></div><ul><li>Protein at each meal when possible.</li><li>Pair carbs with protein around training for energy and recovery.</li><li>Hydrate before gym; add electrolytes if walking/training in heat.</li><li>Keep a quick fallback meal ready so fatigue does not become snack-chaos goblin time.</li></ul></div><div className="panel checkCard"><div className="sectionTitle"><Moon size={20}/><h3>Sleep hygiene</h3></div><ul><li>Same wake time most days.</li><li>Dim screens/bright lights in the final 30–60 minutes.</li><li>Avoid caffeine late afternoon/evening.</li><li>Keep the room cool, dark and boring in the best way.</li></ul></div><div className="panel checkCard"><div className="sectionTitle"><StretchHorizontal size={20}/><h3>Mobility / stretching</h3></div><ul><li>Before training: gentle dynamic warm-up, not aggressive stretching.</li><li>For movement prep: use gentle mobility before lower-body sessions.</li><li>For core: controlled activation before loaded movements works well.</li><li>After training: relaxed stretching and breathing to downshift.</li></ul></div><div className="panel checkCard"><div className="sectionTitle"><AlertTriangle size={20}/><h3>Modify today if…</h3></div><ul><li>Sleep or energy is low.</li><li>Stress is high.</li><li>Focus feels poor.</li><li>Form starts to break down early.</li></ul></div></div><div className="panel"><h2>Menstruation tracking setting</h2><div className="twoCol"><Button variant={trackCycle==="yes"?"primary":"secondary"} onClick={()=>changeCycleTracking("yes")}>Track menstruation</Button><Button variant={trackCycle==="no"?"primary":"secondary"} onClick={()=>changeCycleTracking("no")}>Hide cycle tracking</Button></div></div><div className="panel"><h2>API checks</h2><p className="muted"><code>/api/health</code>, <code>/api/exercises</code>, <code>/api/history</code>, <code>/api/exercise-history?exerciseId=...</code></p></div></section>}
