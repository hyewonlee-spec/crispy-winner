...
export default function App(){
  ...
  function toggleTodaySymptom(key){
    ...
    const setters={backPain:setBackPain,nerve:setNerve,anklePain:setAnklePain,shoulder:setShoulder};
    const current={backPain,nerve,anklePain,shoulder}[key];
    setters[key]?.(Number(current||0)>0?0:6);
  }
  ...
  </main><nav className={`bottomNav ${activeWorkout&&trackCycle==="yes" ? "six" : activeWorkout||trackCycle==="yes" ? "five" : "four"}`}><button className={tab==="today"?"active":""} onClick={()=>setTab("today")}>Today</button>{activeWorkout&&{activeWorkout&&<button className={tab==="workout"?"active":""} onClick={()=>setTab("workout")}>Workout</button>}}<button className={tab==="library"||tab==="exerciseHistory"?"active":""} onClick={()=>setTab("library")}>Library</button><button className={tab==="progress"?"active":""} onClick={()=>setTab("progress")}>Progress</button>{trackCycle==="yes"&&<button className={tab==="cycle"?"active":""} onClick={()=>setTab("cycle")}>Cycle</button>}<button className={tab==="rules"?"active":""} onClick={()=>setTab("rules")}>Check</button></nav></div>
}
